...
export class CliStateManager {
  ...
  static getSessionBudget(provider?: string): number {
    ...
    return state.budgets.default || 50.00;
  }
...
export function createPaginationControls(options: PaginationOptions): string {
  ...
  if (showInfo) {
    ...
    controls += chalk.gray(`📄 Page ${currentPage}/${totalPages} • Items ${startItem}-${endItem} of ${totalItems}\n`);
  }
...
export async function createFilterInterface(currentFilters: FilterOptions): Promise<FilterOptions> {
  ...
  console.log(createBox('🔍 Filter & Sort Options', [
  ...
  const sortAnswer = await inquirer.prompt([{
    ...
    default: currentFilters.sortBy ? `${currentFilters.sortBy}_${currentFilters.sortOrder || 'desc'}` : 'date_desc'
  }]);
...
export async function createSearchInterface(
  ...
  const queryAnswer = await inquirer.prompt([{
    ...
    validate: (input: string) => input.trim().length >= 2 || 'Search query must be at least 2 characters'
  }]);
...
export function filterAndSearchInteractions(
  ...
  if (filters.costRange) {
    filtered = filtered.filter(i =>
      i.costUSD >= filters.costRange!.min && i.costUSD <= filters.costRange!.max
    );
  }
