...
export function middleware(request: NextRequest) {
  ...
  const isProtectedRoute = protectedRoutes.some(route => pathname.startsWith(route)); // eslint-disable-line @typescript-eslint/no-unused-vars
  const isPublicRoute = publicRoutes.some(route => pathname === route); // eslint-disable-line @typescript-eslint/no-unused-vars
